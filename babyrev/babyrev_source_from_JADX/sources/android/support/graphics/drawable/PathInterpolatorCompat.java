...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (numPoints > 0) {
            ...
            if (((double) Math.abs(this.f5mX[0])) > 1.0E-5d || ((double) Math.abs(this.f6mY[0])) > 1.0E-5d || ((double) Math.abs(this.f5mX[numPoints - 1] - 1.0f)) > 1.0E-5d || ((double) Math.abs(this.f6mY[numPoints - 1] - 1.0f)) > 1.0E-5d) {
                throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f5mX[0] + "," + this.f6mY[0] + " end:" + this.f5mX[numPoints - 1] + "," + this.f6mY[numPoints - 1]);
            }
            float prevX = 0.0f;
